...
simulateOutbreakSEIR = function(R0t,rho, R0tpostoutbreak = 1.5,dateEndIntenseIntervention, #date we begin relaxing intense intervention 
                            ...
                            dateStart = as.Date('2019-11-01'),POP = wuhanpop,numWeekStagger=c(2,4,6),pInfected = pInfected,durInf = durInf,contacts_china=contacts)
{
  ...
  kappa = 1-exp(-1/durLat);                                      # infection rate
  dt = 1;                                                        # Time step (days)
  tmax = tEnd;    # 428                                                 # Time horizon (days) 366 days in 2020 cause of leap year
  ...
  { 
    ...
    constraintsIntervention = loadInterventions(p_workopen = pwork[t])
    ...
    # beta = getbeta(R0t = R0t[t],constraints = constraintsIntervention$base,gamma = gamma,p_age = pop$p_age)

    if(time[t] < tEndIntenseIntervention+0)   lambda[t,] = as.numeric(beta)*(as.matrix(C)%*%as.matrix(I[t,]/N_age));
    if(time[t] >= tEndIntenseIntervention+0)  lambda[t,] = as.numeric(beta_postfirstwave)*(as.matrix(C)%*%as.matrix(I[t,]/N_age));
    # lambda[t,] = as.numeric(beta)*(as.matrix(C)%*%as.matrix(I[t,]/N_age));
    # calculate the number of infections and recoveries between time t and t+dt
    
    numInfection = lambda[t,]*S[t,]*dt; # S to E
    numInfected = kappa*E[t,]*dt;               # E to I
    numRecovery = gamma*I[t,]*dt;               # I to R
    numReported = numInfected*rho[t];           # I to H, but not removed from I (remember this is an accumulator function)
    
    # SEIR difference equations 
    S[t+1,] = S[t,]-numInfection;
    E[t+1,] = E[t,]+numInfection-numInfected;
    I[t+1,] = I[t,]+numInfected-numRecovery;
    R[t+1,] = R[t,]+numRecovery;
    H[t+1,] = H[t,]+numReported;
    incidence[t+1,] = numInfected/dt;
    reported[t+1,] = numReported/dt;
    time[t+1] = time[t]+dt;
                                    ...
                                    dateStart = as.Date('2019-11-01'),POP = wuhanpop,numWeekStagger=c(2,4,6),pInfected = pInfected,durInf = durInf,contacts_china=contacts)
{
  ...
  gamma = 1-exp(-1/durInf);                                      # removal rate
  kappa = 1-exp(-1/durLat);                                      # infection rate
  alpha = 0.25;                                                  # relative infectiveness of subclinical cases
  dt = 1;                                                        # Time step (days)
  tmax = tEnd; # 428                                                  # Time horizon (days) 366 days in 2020 cause of leap year
  numSteps = tmax/dt;  	                                         # Total number of simulation time steps
  # dateStart = as.Date('2019-12-01')                            # included as a function argument 
  ...
  # tStartEndClosure = as.vector(dateEndSchoolClosure - dateStart)+1
  pwork = array(1,numSteps)
  # array of proportion of the work force that is working
  pwork[1:tRelaxIntervention3] =c(rep(1,(tStartIntenseIntervention-0)), # dont know there is outbreak 
                                  rep(pWorkOpen[1],(tEndIntenseIntervention-tStartIntenseIntervention)),
                                  rep(pWorkOpen[2],(tRelaxIntervention1-tEndIntenseIntervention)),
                                  rep(pWorkOpen[3],(tRelaxIntervention2-tRelaxIntervention1)),
  ...
  { 
    ...
    # beta = getbeta(R0t = R0t[t],constraints = constraintsIntervention$base,gamma = gamma,p_age = pop$p_age)
    if(time[t] < tEndIntenseIntervention+0) lambda[t,] = as.numeric(beta)*(as.matrix(C)%*%(as.matrix(Ic[t,]/N_age) + alpha*as.matrix(Isc[t,]/N_age)));
    if(time[t] >= tEndIntenseIntervention+0)lambda[t,] = as.numeric(beta_postfirstwave)*(as.matrix(C)%*%(as.matrix(Ic[t,]/N_age) + alpha*as.matrix(Isc[t,]/N_age)));
    # calculate the number of infections and recoveries between time t and t+dt
    
    numStoE   = lambda[t,]*S[t,]*dt;                  # S to E
    numEtoIc  = kappa*rho*E[t,]*dt;                           # E to Ic
    numEtoIsc = kappa*(1-rho)*E[t,]*dt;                       # E to Isc
    numIctoR  = gamma*Ic[t,]*dt;                              # Ic to R
    numIsctoR = gamma*Isc[t,]*dt;                             # Isc to R
    
    # Difference equations 
    S[t+1,]   = S[t,]-numStoE;
    E[t+1,]   = E[t,]+numStoE-numEtoIc-numEtoIsc;
    Ic[t+1,]  = Ic[t,]+numEtoIc-numIctoR;
    Isc[t+1,] = Isc[t,]+numEtoIsc-numIsctoR;
    R[t+1,]   = R[t,]+numIctoR+numIsctoR;
    
    incidence[t+1,] = numEtoIc/dt;
    subclinical[t+1,] = numEtoIsc/dt;
    time[t+1] = time[t]+dt;
